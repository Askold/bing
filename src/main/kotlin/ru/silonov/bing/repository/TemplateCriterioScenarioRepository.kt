...
interface TemplateCriterioScenarioRepository : JpaRepository<TemplateCriteriaScenario, UUID> {
    ...
    @Query("select tcs from TemplateCriteriaScenario tcs where tcs.templateId.id = :templateId")
    fun findAllByTemplateId(templateId: UUID): List<TemplateCriteriaScenario>

    @Modifying
    @Query("delete from TemplateCriteriaScenario tcs where tcs.templateId.id = :templateId")
    fun deleteAllByTemplateId(templateId: UUID)
}